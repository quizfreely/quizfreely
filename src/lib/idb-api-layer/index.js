...
export const idbApiLayer = {
    ...
    getReviewEventStatsByDay: async function ({ studysetId, termIds, lastDaysBack, lastDaysTotal } = {}) {
        ...
        if (hasLastDaysBack && hasLastDaysTotal) {
            throw new Error("(idbApiLayer.getReviewEventStatsByDay) only one of lastDaysBack or lastDaysTotal may be provided");
        }
        if (!hasLastDaysBack && !hasLastDaysTotal) {
        ...
        const oldestDayStart = hasLastDaysBack
            ? new Date(now.getFullYear(), now.getMonth(), now.getDate() - (lastDaysBack - 1)).getTime()
            : null;
    ...
    activityHistory: async function ({ last, getCloudStudysets }) {
        ...
        const incorrectPairsByMatchId = new Map();
        ...
        const localStudysetIds = new Set();
        const cloudStudysetIds = [];
        ...
        if (cloudStudysetIds.length > 0 && !getCloudStudysets) {
            throw new Error("(idbApiLayer.activityHistory) cloud studyset UUIDs found but no getCloudStudysets callback provided. " +
        ...
        if (cloudStudysetIds.length > 0 && getCloudStudysets) {
            ...
            for (let i = 0; i < cloudStudysetIds.length; i++) {
                ...
                if (s != null)
                    cloudStudysetMap.set(cloudStudysetIds[i], s);
            }
        }
